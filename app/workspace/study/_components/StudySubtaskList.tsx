...
import StudySubtaskDropdownMenu from './StudySubtaskDropdownMenu';
...
interface StudySubtasksListProps {
  subtasks: Task[];
  onSubtaskChanged?: (updatedSubtasks: Task[]) => void;
}

export default function StudySubtasksList({ subtasks, onSubtaskChanged }: StudySubtasksListProps) {
  ...
  useEffect(() => {
    console.log('Current study subtasks with statuses:', localSubtasks.map(t => ({ id: t.id, title: t.title, status: t.status })));
  }, [localSubtasks]);
  ...
  const toggleSubtaskComplete = async (taskId: number, currentStatus: string) => {
    ...
    console.log(`Toggling study subtask ${taskId} from ${currentStatus} to ${newStatus}`);
    ...
    if (success) {
      ...
      console.log('Updated local study subtasks:', updatedSubtasks.map(t => ({ id: t.id, status: t.status })));
  ...
  const handleEditSubtask = async (
    ...
    const success = await handleUpdateTask(taskId, title, description, subjectId, parentId, dueDate);
